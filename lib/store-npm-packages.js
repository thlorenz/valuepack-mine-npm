...
var get = module.exports = function (db, packages, cb) {
  ...
  npmPackages.pre(function (val, add) {
    var packname = val.key;
    try {
      add({ prefix        :  byOwner
          , type          :  'put'
          , key           :  val.value.owner + '\xff' + packname
          , value         :  packname
          , valueEncoding :  'utf8'
      });
    } catch(e) {
  ...
  npmPackages.batch(
      packagesBatch
    , function (err) {
        cb(err, { sublevels: { npmPackages: npmPackages, byOwner: byOwner, byKeyword: byKeyword } });
    });
};
